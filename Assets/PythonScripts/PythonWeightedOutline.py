...
class FrameHumanAnalysis:
    ...
    def analyze_frame(self, layer_count: int = 3) -> np.ndarray:
        human_mask = self.get_human()
        human_outline_mask = self.get_human_outline(human_mask)

        human_outline_colored = np.zeros_like(self.__orig_cv)
        human_outline_colored[human_outline_mask == 1] = self.__outline_color
        
        return human_outline_colored
